...
let score = 0;
    ...
    function renderCategoriesTree() {
        const categoriesContainer = document.getElementById('categories-container'); // Add this line
        categoriesContainer.innerHTML = ''; // Add this line
        ...
        function createCategoryElement(category) {
            ...
            div.innerHTML = `
                <div class="category-item-header">
                    ...
                    <div class="category-actions">
                        <button class="category-action-btn add">+</button>
                        <button class="category-action-btn delete">×</button>
                    </div>
                </div>
                ${category.children.length > 0 ? '<div class="category-children"></div>' : ''}
            `;
            ...
            addBtn.addEventListener('click', (e) => {
                ...
                if (name) {
                    categoryManager.addCategory(name, category.id);
            ...
            if (category.children.length > 0) {
                const childrenContainer = div.querySelector('.category-children');
                category.children.forEach(child => {
                    childrenContainer.appendChild(createCategoryElement(child));
                });
            }
        ...
        // Rendre toutes les catégories racines
    ...
    function init() {
        ...
        // Render the categories tree (add this line)
        renderCategoriesTree(); // Add this line
    }
});The `categoryManager` object and `categories-container` element are assumed to be defined elsewhere in the code.  This solution adds the `renderCategoriesTree` function and calls it in `init`.
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function fuzzySearch(cleanedQuery) {
        const results = [];
        const queryWords = cleanedQuery.split(' ');        notes.forEach(note => {
            let score = 0;
            const cleanTitle = cleanText(note.title || '');
            const cleanContent = cleanText(note.content || '');
            const titleWords = cleanTitle.split(' ');
